...
public class Panel extends javax.swing.JPanel {    
    ...
    public void paint (Graphics g) {
        ...
        NodeGrid grid = new NodeGrid( tree.size(), 5, 15 ).build( tree, g, font );

        for( NodeGrid.DrawableNode node : grid.nodes ){
            drawNode( g, node, grid );
        }
    }
    ...
    private void drawNode( Graphics g, NodeGrid.DrawableNode node, NodeGrid grid ){

        Node act = node.node;
        int x = node.point.x * grid.cellWidth  + node.point.x * grid.sepWidth;
        int y = node.point.y * grid.cellHeight + node.point.y * grid.sepHeight;
        

        // I think a rectangle
        g.setColor(Color.WHITE);
        // g.fillOval( node.point.x, node.point.y, 50, 50);
        g.fillRect( x, y, grid.cellWidth, grid.cellHeight );

        // Draw name and duration.
        g.setColor(Color.BLACK);
        g.drawString( act.getActivityName(), x + 3, y + grid.cellHeight/2 );
        g.drawString( ""+act.getActivityDuration(), x + 3, y + grid.cellHeight );

        // Hmm idunno, still rectangle
        g.setColor( Color.BLUE );
        g.drawRect( x, y, grid.cellWidth, grid.cellHeight );
    }
